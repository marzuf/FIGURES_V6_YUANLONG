# Rscript range_fcc_v2_either_iterate.R

outFolder <- "RANGE_FCC_V2_EITHER_ITERATE"
...
if(buildData){
  ...
  all_fccRange_dt <- foreach(hicds = all_hicds, .combine='rbind') %dopar% {
    ...
    hicds_dt <- foreach(exprds = all_exprds[[paste0(hicds)]], .combine='rbind') %do% {
      ...
      rd_fcc_hist <- hist(all_fcc_v2_either, breaks=fcc_fract)$counts
      names(rd_fcc_hist) <- fcc_fract_names
      
      rd_fcc_hist <- rd_fcc_hist/length(all_fcc_v2_either)
...
# load("RANGE_FCC_V2_EITHER_ITERATE/all_fccRange_dt.Rdata")
...
# same but log2
all_fccRange_dt$obs_over_rd_log2 <- log2(all_fccRange_dt$obs_over_rd) 
box_p <- ggboxplot(data = all_fccRange_dt,
                   x = "intervalFCC", y = "obs_over_rd_log2",
                   xlab="FCC threshold", ylab="Ratio TADs FCC in range [log2]",
                   title = "V2_EITHER")+
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10))+
